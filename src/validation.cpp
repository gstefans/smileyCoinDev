...
const std::string strMessageMagic = "Smileycoin Signed Message:\n";
...
CAmount GetBlockSubsidy(int nHeight, const Consensus::Params& consensusParams)
{
    ...
    if (nHeight <= consensusParams.PremineEndHeight) {
        nSubsidy = consensusParams.nPremineSubsidyAmount * COIN;
    }
    // Subsidy is cut in half every 1,226,400 blocks which will occur approximately every 7 years.
    nSubsidy >>= halvings;
    return nSubsidy;
}
...
void ThreadScriptCheck() {
    RenameThread("smileycoin-scriptch");
...
static bool ContextualCheckBlockHeader(const CBlockHeader& block, CValidationState& state, const CChainParams& params, const CBlockIndex* pindexPrev, int64_t nAdjustedTime)
{
    ...
    if (block.nBits != GetNextWorkRequired(pindexPrev, &block, consensusParams)) {
        // [smly] Disabled as the current version of GetNextMultiAlgoWorkRequired doesn't work
        // This mirrors the behaviour of the version 2 wallet but should ideally be fixed.
    ...
    // [smly] We cannot rely on block.nVersion as that has been co-opted by multialgo.
